...
import { FaPencilAlt, FaSquare, FaCircle, FaRegTrashAlt, FaSave, FaFileDownload, FaSearchPlus, FaSearchMinus, FaUndo } from 'react-icons/fa';
...
const PageContainer = styled.div`
  ${tw`min-h-screen p-8 bg-gray-100`}
`;

// Enhanced button styling for vibrant color palette and shadow effects
const ActionButton = styled.button`
  ${tw`flex items-center justify-center px-4 py-2 m-2 text-white rounded-md focus:outline-none transition-transform duration-200 ease-in-out transform hover:scale-105`}
  background-color: #6415FF; // Purple color
  &:hover {
    background-color: #5a4bcc;
  }
`;

// Header styling for typography enhancements
const PageTitle = styled.h1`
  ${tw`text-2xl font-bold text-left text-gray-800 mb-8`}
`;

// Input and Label styling for readability and alignment with the theme
const LabelInput = styled.input`
  ${tw`w-full p-2 mb-4 border border-gray-300 rounded-md shadow-sm`}
`;

// Container for canvas to ensure it is the focal point
const CanvasContainer = styled.div`
  ${tw`relative w-full max-w-4xl p-4 mx-auto my-8 border border-solid border-gray-200 rounded-lg shadow-sm bg-white`}
  canvas {
    ${tw`border-2 border-solid border-gray-300`}
  }
`;

// Control panel for intuitive download and zoom functions
const ControlsContainer = styled.div`
  ${tw`flex items-center justify-start p-4 bg-white border-t border-gray-200`}
`;
const ShapeButton = styled(ActionButton)`
...
const AnnotateWSIPatch = () => {
  ...
  const { patchImage } = location.state;
  const PageTitle = styled.h1`
  font-size: 2rem;
  font-weight: 900;
  color: #6f18f2;
...
useEffect(() => {
  ...
  const saveAnnotatedImage = () => {
    if (canvas) {
      ...
      downloadImage(imageData, `${imageID}.png`);
...
const addLine = () => {
  ...
  useEffect(() => {
    ...
    if (canvasRef.current && patchImage) {
  ...
  return (
    <PageContainer>
      <Header /> 
      <h1 style={{ fontSize: '32px', fontWeight: 'bold' }}>WSI Patch Annotation</h1> {/* Bigger and bold heading */}
...
<div>
      ...
      <div style={{ position: 'absolute', right: '0px', top: '110px', width: '480px', maxHeight: '800px', overflowY: 'auto', border: '1px solid #ccc', padding: '4px', borderRadius: '5px', backgroundColor: '#f9f9f9', zIndex: '1', marginRight:'10px' }}>
...
export default AnnotateWSIPatch;